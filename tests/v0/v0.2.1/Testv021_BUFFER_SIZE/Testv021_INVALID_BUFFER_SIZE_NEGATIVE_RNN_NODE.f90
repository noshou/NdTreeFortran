!> Expected-fail: rNN_Node with bufferSize=-1 must error stop.
!! Registered with WILL_FAIL in CTest.
program Testv021_INVALID_BUFFER_SIZE_NEGATIVE_RNN_NODE
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none

    call invalidBufferSizeNegative_rNN_Node()
    contains

        subroutine invalidBufferSizeNegative_rNN_Node()
            type(KdTree)                 :: t
        ...
        end subroutine invalidBufferSizeNegative_rNN_Node

end program Testv021_INVALID_BUFFER_SIZE_NEGATIVE_RNN_NODE
