program Testv021_EMPTY_RESULT_POPULATED_TREE_RNN_NODE_CHEBYSHEV
    ...
    call emptyResultPopulatedTree_rNN_Centroid_Chebyshev()
    contains

        !> Query should return no nodes; tree is populated
        subroutine emptyResultPopulatedTree_rNN_Centroid_Chebyshev()
            type(KdTree)                 :: t
            ...
            if (size(res) .ne. 0) then 
                write(*, '(A)') '--- emptyResultPopulatedTree_rNN_Centroid_Chebyshev ---'
                write(*, *) 'expected 0 nodes, got:', size(res)
                stop 1
            end if

        end subroutine emptyResultPopulatedTree_rNN_Centroid_Chebyshev
