...
program Testv021_EMPTY_RESULT_POPULATED_TREE_RNN_CENTROID_MANHATTAN
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none

    call emptyResultPopulatedTree_rNN_Centroid_Manhattan()
    contains

        !> Query should return no nodes; tree is populated
        subroutine emptyResultPopulatedTree_rNN_Centroid_Manhattan()
            type(KdTree)                 :: t
            ...
            call t%build(coords)
            res = t%rNN_Centroid(centroid, r, metric='manhattan')
            if (size(res) .ne. 0) then 
                write(*, '(A)') '--- emptyResultPopulatedTree_rNN_Centroid_Manhattan ---'
                write(*, *) 'expected 0 nodes, got:', size(res)
                stop 1
            end if

        end subroutine emptyResultPopulatedTree_rNN_Centroid_Manhattan
end program Testv021_EMPTY_RESULT_POPULATED_TREE_RNN_CENTROID_MANHATTAN