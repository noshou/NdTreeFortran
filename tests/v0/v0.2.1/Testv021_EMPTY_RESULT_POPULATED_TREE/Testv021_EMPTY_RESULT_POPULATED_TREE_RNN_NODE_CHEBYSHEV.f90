program Testv021_EMPTY_RESULT_POPULATED_TREE_RNN_NODE_CHEBYSHEV
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none
    call emptyResultPopulatedTree_rNN_Node_Chebyshev()
    contains 

        !> Query should return no nodes; tree is populated
        subroutine emptyResultPopulatedTree_rNN_Node_Chebyshev()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Node(centroid_res(1), 0.000000000000031_real64, metric='chebyshev', excludeTarget=.true.)
            if (size(res) .ne. 0) then
                write(*, '(A)') '--- emptyResultPopulatedTree_rNN_Node_Chebyshev ---'
                write(*,*) 'expected 0 nodes, got:', size(res)
                stop 1
            end if

        end subroutine emptyResultPopulatedTree_rNN_Node_Chebyshev

end program Testv021_EMPTY_RESULT_POPULATED_TREE_RNN_NODE_CHEBYSHEV