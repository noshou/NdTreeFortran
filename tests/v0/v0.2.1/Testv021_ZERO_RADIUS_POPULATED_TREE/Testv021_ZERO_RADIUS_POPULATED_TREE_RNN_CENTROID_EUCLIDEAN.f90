...
program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_CENTROID_EUCLIDEAN
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none

    call zeroRadiusPopulatedTree_rNN_Centroid_Euclidean()
    contains

        !> Query should return no nodes; tree is populated
        subroutine zeroRadiusPopulatedTree_rNN_Centroid_Euclidean()
            type(KdTree)                 :: t
            ...
            if (size(res) .ne. 0) then 
                write(*, '(A)') '--- zeroRadiusPopulatedTree_rNN_Centroid_Euclidean ---'
                write(*, *) 'expected 0 nodes, got:', size(res)
                stop 1
            end if

        end subroutine zeroRadiusPopulatedTree_rNN_Centroid_Euclidean
end program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_CENTROID_EUCLIDEAN