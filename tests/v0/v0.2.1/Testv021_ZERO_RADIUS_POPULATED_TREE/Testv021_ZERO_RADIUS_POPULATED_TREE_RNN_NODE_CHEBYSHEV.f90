program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_NODE_CHEBYSHEV
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none
    call zeroRadiusPopulatedTree_rNN_Node_Chebyshev()
    contains 

        !> Query should return one node (target node); tree is populated
        subroutine zeroRadiusPopulatedTree_rNN_Node_Chebyshev()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Node(centroid_res(1), 0.0_real64, metric='chebyshev')
            if (size(res) .ne. 1) then
                write(*, '(A)') '--- zeroRadiusPopulatedTree_rNN_Node_Chebyshev ---'
                write(*,*) 'expected 1 node, got:', size(res)
                stop 1
            end if

        end subroutine zeroRadiusPopulatedTree_rNN_Node_Chebyshev

end program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_NODE_CHEBYSHEV