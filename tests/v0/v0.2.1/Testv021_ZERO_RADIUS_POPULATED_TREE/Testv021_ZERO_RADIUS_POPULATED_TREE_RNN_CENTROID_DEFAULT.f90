...
program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_CENTROID_DEFAULT
    ...
    call zeroRadiusPopulatedTree_rNN_Centroid_Default()
    contains
        ...
        subroutine zeroRadiusPopulatedTree_rNN_Centroid_Default()
            ...
            if (size(res) .ne. 0) then 
                write(*, '(A)') '--- zeroRadiusPopulatedTree_rNN_Centroid_Default ---'
                write(*, *) 'expected 0 nodes, got:', size(res)
                stop 1
            end if

        end subroutine zeroRadiusPopulatedTree_rNN_Centroid_Default
end program Testv021_ZERO_RADIUS_POPULATED_TREE_RNN_CENTROID_DEFAULT