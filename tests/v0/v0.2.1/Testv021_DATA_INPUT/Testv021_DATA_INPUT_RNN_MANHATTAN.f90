program Testv021_DATA_INPUT_RNN_MANHATTAN
    use NdTreeFortran
    use iso_fortran_env, only: real64

    call dataInput_rNN_Manhattan()
    contains 

        !> expected to return exactly two nodes with data "4" and "6"
        subroutine dataInput_rNN_Manhattan()
            type(KdTree)                 :: t
            ...
            r = sum(abs([0.0_real64, 0.0_real64, 0.00000031_real64] - [0.0_real64, 0.0_real64, 0.0_real64]))
            res = t%rNN_Centroid([0.0_real64, 0.0_real64, 0.0_real64], r, metric='manhattan')
                ...
                select type (d => res(i)%p%getData())
                    type is (character(*))
                        if ((d .eq. '4') .and. (.not. found4)) then
                            ...
                            write(*, '(A)') '--- dataInput_rNN_Manhattan ---'
        ...
        end subroutine dataInput_rNN_Manhattan

end program Testv021_DATA_INPUT_RNN_MANHATTAN