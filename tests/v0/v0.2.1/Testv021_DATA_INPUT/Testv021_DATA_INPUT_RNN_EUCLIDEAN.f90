program Testv021_DATA_INPUT_RNN_EUCLIDEAN
    use NdTreeFortran
    use iso_fortran_env, only: real64

    call dataInput_rNN_Euclidean()
    contains 
        ...
        subroutine dataInput_rNN_Euclidean()
            ...
            res = t%rNN_Centroid([0.0_real64, 0.0_real64, 0.0_real64], r, metric='euclidean')
                ...
                select type (d => res(i)%p%getData())
                    type is (character(*))
                        if ((d .eq. '4') .and. (.not. found4)) then
                            ...
                            write(*, '(A)') '--- dataInput_rNN_Euclidean ---'
        ...
        end subroutine dataInput_rNN_Euclidean

end program Testv021_DATA_INPUT_RNN_EUCLIDEAN