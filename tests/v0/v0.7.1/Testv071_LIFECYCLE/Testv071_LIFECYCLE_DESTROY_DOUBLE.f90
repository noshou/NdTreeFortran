program Testv071_LIFECYCLE_DESTROY_DOUBLE
    use NdTreeFortran
    ...
    contains
        ...
        !> checks if "double destroy" works correctly, internal state is
        !! as expected, and no errors are caused.
        subroutine lifecycleDestroy()
            ...
            if (assertNodePool .or. assertRoot .or. assertInitState) then
                write(*, '(A)')    '--- Testv071_LIFECYCLE_DESTROY_DOUBLE ---'
                write(*, '(A)')    'expected: '
...
end program Testv071_LIFECYCLE_DESTROY_DOUBLE
