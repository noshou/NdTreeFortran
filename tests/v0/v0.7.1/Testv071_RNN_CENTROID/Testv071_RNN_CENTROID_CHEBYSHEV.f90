!> BallTree rNN_Centroid with metric='chebyshev'.
...
program Testv071_RNN_CENTROID_CHEBYSHEV
    ...
    contains
        ...
        subroutine run()
            ...
            if (size(res) .ne. 3) then
                write(*, '(A)') '--- Testv071_RNN_CENTROID_CHEBYSHEV ---'
                write(*,*) 'expected 3 nodes, got:', size(res)
                stop 1
            end if
        end subroutine run

end program Testv071_RNN_CENTROID_CHEBYSHEV
