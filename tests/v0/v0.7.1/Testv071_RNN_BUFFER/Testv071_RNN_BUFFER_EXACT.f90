!> BallTree rNN_Centroid where bufferSize equals the result count exactly -> no resize should occur.
program Testv071_RNN_BUFFER_EXACT
    use NdTreeFortran
    ...
    contains
        ...
        subroutine run()
            ...
            do i = 1, 1000
                res = t%rNN_Centroid(centroid, 1.0_real64, bufferSize=10_int64)
                if (size(res) .ne. 10) then
                    write(*, '(A)') '--- Testv071_RNN_BUFFER_EXACT ---'
...
end program Testv071_RNN_BUFFER_EXACT
