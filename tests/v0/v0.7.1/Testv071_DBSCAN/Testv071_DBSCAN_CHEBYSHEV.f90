program Testv071_DBSCAN_CHEBYSHEV
    use NdTreeFortran
    ...
    contains
        ...
        !! BallTree fixes metric at build, so setMetric('chebyshev') before build.
        subroutine dbscanChebyshev()
            ...
            call t%build(coords)
            res = t%DBSCAN(minPts=2_int64, radius=1.0_real64, metric='chebyshev')

            nClusters = size(res) - 1
            noiseSize = size(res(size(res))%nodes)

            if (nClusters .ne. 1) then
                write(*, '(A)')    '--- Testv071_DBSCAN_CHEBYSHEV ---'
...
end program Testv071_DBSCAN_CHEBYSHEV
