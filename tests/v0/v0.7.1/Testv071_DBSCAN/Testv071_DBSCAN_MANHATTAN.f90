program Testv071_DBSCAN_MANHATTAN
    use NdTreeFortran
    ...
    contains
        ...
        !! BallTree fixes metric at build, so setMetric('manhattan') before build.
        subroutine dbscanManhattan()
            ...
            if (nClusters .ne. 0) then
                write(*, '(A)')    '--- Testv071_DBSCAN_MANHATTAN ---'
...
end program Testv071_DBSCAN_MANHATTAN
