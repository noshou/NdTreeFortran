program Testv030_ADD_NODES_RNN_NODE_FINDS_ADDED_CHEBYSHEV
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none
    call rnnNodeFindsAddedChebyshev()
    contains
        !> Build tree with (0,0),(10,0),(0,10). Add (1,0),(0,1).
        !! rNN_Node from (0,0) with r=1.5 (chebyshev) finds (0,0),(1,0),(0,1) = 3 nodes.
        subroutine rnnNodeFindsAddedChebyshev()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Node(target, 1.5_real64, metric='chebyshev')

            if (size(res) .ne. 3) then
                write(*, '(A)')   '--- Testv030_ADD_NODES_RNN_NODE_FINDS_ADDED_CHEBYSHEV ---'
                write(*, '(A,I0)') 'expected 3 nodes, got: ', size(res)
                stop 1
            end if
        end subroutine rnnNodeFindsAddedChebyshev
end program Testv030_ADD_NODES_RNN_NODE_FINDS_ADDED_CHEBYSHEV
