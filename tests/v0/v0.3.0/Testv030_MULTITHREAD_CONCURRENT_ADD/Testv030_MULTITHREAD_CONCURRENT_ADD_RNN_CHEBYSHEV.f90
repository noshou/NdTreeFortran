program Testv030_MULTITHREAD_CONCURRENT_ADD_RNN_CHEBYSHEV
    ...
    contains
        subroutine concurrentAddRnnChebyshev()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Centroid([6.5_real64, 0.0_real64], 7.0_real64, metric='chebyshev')
            if (size(res) .ne. 13) then
                write(*, '(A)')   '--- Testv030_MULTITHREAD_CONCURRENT_ADD_RNN_CHEBYSHEV ---'
                write(*, '(A,I0)') 'expected 13 nodes, got: ', size(res)
                stop 1
            end if
        end subroutine concurrentAddRnnChebyshev
end program Testv030_MULTITHREAD_CONCURRENT_ADD_RNN_CHEBYSHEV
