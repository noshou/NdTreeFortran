    ...
    contains
        ...
        subroutine incrementalRnnChebyshev()
            ...
            do k = 1, 5
                ...
                res = t%rNN_Centroid([step(1,1), 0.0_real64], 0.5_real64, metric='chebyshev')
                if (size(res) .ne. 1) then
                    write(*, '(A)')      '--- Testv030_ADD_NODES_INCREMENTAL_RNN_CHEBYSHEV ---'
                    write(*, '(A,I0,A,I0)') 'after adding node ', k, ' expected rNN size=1, got: ', size(res)
                    stop 1
                end if
            end do
        end subroutine incrementalRnnChebyshev
end program Testv030_ADD_NODES_INCREMENTAL_RNN_CHEBYSHEV
