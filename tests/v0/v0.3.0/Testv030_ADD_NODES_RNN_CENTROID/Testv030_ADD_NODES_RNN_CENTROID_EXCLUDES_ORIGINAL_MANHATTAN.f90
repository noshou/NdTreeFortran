    ...
    contains
        ...
        subroutine rnnCentroidExcludesOriginalManhattan()
            ...
            if (size(res) .ne. 3) then
                write(*, '(A)')   '--- Testv030_ADD_NODES_RNN_CENTROID_EXCLUDES_ORIGINAL_MANHATTAN ---'
                write(*, '(A,I0)') 'expected 3 nodes (original only), got: ', size(res)
                stop 1
            end if
        end subroutine rnnCentroidExcludesOriginalManhattan
end program Testv030_ADD_NODES_RNN_CENTROID_EXCLUDES_ORIGINAL_MANHATTAN
