program Testv030_ADD_NODES_MULTI_ADD_RNN_MANHATTAN
    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none
    call multiAddRnnManhattan()
    contains
        !> Three addNodes calls add pts at x=1..9, y=0. All 10 pts are searchable.
        !! rNN_Centroid at (4.5,0) r=5 (manhattan) must find all 10 pts.
        subroutine multiAddRnnManhattan()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Centroid([4.5_real64, 0.0_real64], 5.0_real64, metric='manhattan')

            if (size(res) .ne. 10) then
                write(*, '(A)')   '--- Testv030_ADD_NODES_MULTI_ADD_RNN_MANHATTAN ---'
                write(*, '(A,I0)') 'expected 10 nodes, got: ', size(res)
                stop 1
            end if
        end subroutine multiAddRnnManhattan
end program Testv030_ADD_NODES_MULTI_ADD_RNN_MANHATTAN
