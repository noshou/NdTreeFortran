program Testv030_ADD_NODES_MULTI_ADD_RNN_CHEBYSHEV
    ...
    contains
        ...
        !! rNN_Centroid at (4.5,0) r=5 (chebyshev) must find all 10 pts.
        subroutine multiAddRnnChebyshev()
            type(KdTree)                 :: t
            ...
            res = t%rNN_Centroid([4.5_real64, 0.0_real64], 5.0_real64, metric='chebyshev')

            if (size(res) .ne. 10) then
                write(*, '(A)')   '--- Testv030_ADD_NODES_MULTI_ADD_RNN_CHEBYSHEV ---'
                write(*, '(A,I0)') 'expected 10 nodes, got: ', size(res)
                stop 1
            end if
        end subroutine multiAddRnnChebyshev
end program Testv030_ADD_NODES_MULTI_ADD_RNN_CHEBYSHEV
