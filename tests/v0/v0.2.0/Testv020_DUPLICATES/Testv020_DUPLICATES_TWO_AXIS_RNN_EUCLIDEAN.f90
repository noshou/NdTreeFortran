program Testv020_DUPLICATES_TWO_AXIS_RNN_EUCLIDEAN
    ...
    call duplicatesTwoAxis_rNN_Euclidean()
    contains
        ...
        subroutine duplicatesTwoAxis_rNN_Euclidean()
            ...
            if (size(res) .ne. 9) then
                write(*, '(A)') '--- Testv020_DUPLICATES_TWO_AXIS_RNN_EUCLIDEAN ---'
                write(*, *) 'expected 9 nodes, got:', size(res)
                stop 1
            end if
        end subroutine duplicatesTwoAxis_rNN_Euclidean
end program Testv020_DUPLICATES_TWO_AXIS_RNN_EUCLIDEAN
