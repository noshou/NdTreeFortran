program Testv020_COLLINEAR_ONE_AXIS_II_RNN_CHEBYSHEV
    ...
    call collinearOne_AxisII_rNN_Chebyshev()
    contains
        !> 3D, all y=4. rNN_Centroid at (13,4,0) r=12.5, chebyshev.
        !! P1=(1,4,0.92): dist=max(12,0,0.92)=12 in. P3=(13,4,0.00003): dist≈0 in.
        !! Others: dist>12.5. Expect 2.
        subroutine collinearOne_AxisII_rNN_Chebyshev()
            type(KdTree)                 :: t
            ...
            if (size(res) .ne. 2) then
                write(*, '(A)') '--- Testv020_COLLINEAR_ONE_AXIS_II_RNN_CHEBYSHEV ---'
                write(*, *) 'expected 2 nodes, got:', size(res)
                stop 1
            end if
        end subroutine collinearOne_AxisII_rNN_Chebyshev
end program Testv020_COLLINEAR_ONE_AXIS_II_RNN_CHEBYSHEV
