!> Expected-fail: chebyshevDist between a 2D node and a 3D node must error stop.
!! Registered with WILL_FAIL in CTest.
program Testv020_NODE_QUERY_CHEBYSHEV_AXIS_MISMATCH

    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none

    call nodeQuery_Chebyshev_AxisMismatch()
    contains

        subroutine nodeQuery_Chebyshev_AxisMismatch()
            type(KdTree)                 :: t2, t3
            ...
            type(NdNodePtr), allocatable :: res2(:), res3(:)
            real(real64)               :: dist
...
end program Testv020_NODE_QUERY_CHEBYSHEV_AXIS_MISMATCH
