!> Expected-fail: rNN_Node (euclidean metric) with a node from a different Tree must error stop.
!! Registered with WILL_FAIL in CTest.
program Testv020_SINGLE_NODE_QUERY_RNN_NODE_EUCLIDEAN_NON_MEMBER

    use NdTreeFortran
    use iso_fortran_env, only: real64
    implicit none

    call singleNodeQuery_rNN_Node_Euclidean_NonMember()
    contains

        subroutine singleNodeQuery_rNN_Node_Euclidean_NonMember()
            type(KdTree)                 :: t1, t2
            ...
            write(*, '(A)') '--- Testv020_SINGLE_NODE_QUERY_RNN_NODE_EUCLIDEAN_NON_MEMBER ---'
            write(*, '(A)') 'expected error stop, but rNN_Node returned normally'
        end subroutine singleNodeQuery_rNN_Node_Euclidean_NonMember

end program Testv020_SINGLE_NODE_QUERY_RNN_NODE_EUCLIDEAN_NON_MEMBER
