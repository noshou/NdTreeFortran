!> Expected-fail: rNN_Node (manhattan metric) with a node from a different Tree must error stop.
...
program Testv020_SINGLE_NODE_QUERY_RNN_NODE_MANHATTAN_NON_MEMBER
    ...
    call singleNodeQuery_rNN_Node_Manhattan_NonMember()
    contains
        ...
        subroutine singleNodeQuery_rNN_Node_Manhattan_NonMember()
            ...
            res = t1%rNN_Node(centroid_res(1), 1.0_real64, metric='manhattan')
            write(*, '(A)') '--- Testv020_SINGLE_NODE_QUERY_RNN_NODE_MANHATTAN_NON_MEMBER ---'
            write(*, '(A)') 'expected error stop, but rNN_Node returned normally'
        end subroutine singleNodeQuery_rNN_Node_Manhattan_NonMember

end program Testv020_SINGLE_NODE_QUERY_RNN_NODE_MANHATTAN_NON_MEMBER
